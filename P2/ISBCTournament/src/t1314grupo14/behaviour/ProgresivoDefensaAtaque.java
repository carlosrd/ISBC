...
public class ProgresivoDefensaAtaque extends Behaviour {
	...
	public int takeStep() {
		...
		if (myRobotAPI.getTimeStamp() < 30000)
			accionDefensaAgresiva();					// Si llevamos menos de 30 seg -> Defensa Agresivo
		else if (myRobotAPI.getTimeStamp() < 60000)
				accionDefensa();						// Entre 30-60 seg -> Defensa
		else if (myRobotAPI.getTimeStamp() < 90000) 
				accionAtaqueAgresivo();					// Entre 1min y 1m30 seg -> Ataque Agresivo
		else 
			accionAtaque();								// Entre 1m30 y 2m -> Ataque
